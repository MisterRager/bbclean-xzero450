...
int agenttype_systemmonitor_startup()
{
	...
	if(isnt2kxp)
	{
		...
		agenttype_systemmonitor_ntdllmodule = LoadLibrary((LPCWSTR)"ntdll.dll");
...
void agenttype_systemmonitor_updatevalue(int monitor_type)
{
	...
	switch(monitor_type)
	{
		...
		else
		{
			...
			RegOpenKeyEx(HKEY_DYN_DATA, (LPCWSTR)"PerfStats\\StatData", 0, KEY_QUERY_VALUE, &hkey);
			dwDataSize = sizeof(dwCpuUsage);
			RegQueryValueEx(hkey, (LPCWSTR)"KERNEL\\CPUUsage", NULL, &dwType, (LPBYTE)&dwCpuUsage, &dwDataSize);
			RegCloseKey(hkey);
