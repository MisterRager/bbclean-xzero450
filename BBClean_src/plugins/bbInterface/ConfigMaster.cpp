...
int config_backup(char *filename)
{
	...
	if (!FileExists(bakfile) || IDYES == BBMessageBox(NULL,
		...
		CopyFile((LPCWSTR)filename, (LPCWSTR)bakfile, FALSE);
...
{
	char buffer[MAX_PATH];
	return 0 != DeleteFile((LPCWSTR)config_makepath(buffer, filename));
}
...
{
	HANDLE hf; int r;
	hf = CreateFile((LPCWSTR)fn, 0, FILE_SHARE_READ|FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
void config_paths_startup()
{
	...
	GetModuleFileName(plugin_instance_plugin, (LPWCH)temp, MAX_PATH);
