	...
	switch(notifytype)
	{
		case NOTIFY_DRAW:
			...
			if (details->is_icon)
			{
				...
				HICON hIcon = (HICON)LoadImage(plugin_instance_plugin, (LPCWSTR)details->absolute_path, IMAGE_ICON, details->width, details->height, LR_LOADFROMFILE);
...
HICON load_sysicon(char *path, int size)
{
	...
	SHFILEINFO shinfo;
	HIMAGELIST sysimgl = (HIMAGELIST)SHGetFileInfo((LPCWSTR)path, 0, &shinfo, sizeof(SHFILEINFO), cbfileinfo);
