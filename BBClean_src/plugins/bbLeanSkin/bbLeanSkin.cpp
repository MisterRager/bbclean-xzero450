...
void readSettings(void)
{
	...
	mSkin.focusTransparency = ReadInt(rcpath, "bbleanskin.windows.focusTransparency:", 0);
	if (mSkin.focusTransparency < 0) mSkin.focusTransparency = 0;
	if (mSkin.focusTransparency > 100) mSkin.focusTransparency = 100;

	mSkin.unfocusTransparency = ReadInt(rcpath, "bbleanskin.windows.unfocusTransparency:", 0);
	if (mSkin.unfocusTransparency < 0) mSkin.unfocusTransparency = 0;
	if (mSkin.unfocusTransparency > 100) mSkin.unfocusTransparency = 100;

	mSkin.focusColorTrans = ReadColor(rcpath, "bbleanskin.windows.focusColorTransparency:", "");
	mSkin.unfocusColorTrans = ReadColor(rcpath, "bbleanskin.windows.unfocusColorTransparency:", "");

	//strncpy(mSkin.focusAlphaMethod, ReadString(rcpath, "bbleanskin.windows.focusAlphaMethod:", "none"), 127);
	//strncpy(mSkin.unfocusAlphaMethod, ReadString(rcpath, "bbleanskin.windows.unfocusAlphaMethod:", "none"), 127);

	mSkin.enableShadows = ReadBool(rcpath, "bbleanskin.windows.shadows:", false);
...
static BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam) {
	...
	if ( (isWin2kXP()) && lParam == MSGID_UNLOAD ) {
		//If we're on 2k+ and unloading
		void *pInfo = GetProp(hwnd, BBLEANSKIN_UNFOCUSTRANS);
		void *pInfo2 = GetProp(hwnd, BBSHADE_PROP);

		if ( (GetProp(hwnd, BBLEANSKIN_FOCUSTRANS)) ) {
			SetWindowLong(hwnd, GWL_EXSTYLE,
			GetWindowLong(hwnd, GWL_EXSTYLE) & ~WS_EX_LAYERED);
			RemoveProp(hwnd, BBLEANSKIN_FOCUSTRANS);
		}

		if ( pInfo ) {
			//Only change skinned windows
			SetWindowLong(hwnd, GWL_EXSTYLE,
			GetWindowLong(hwnd, GWL_EXSTYLE) & ~WS_EX_LAYERED);
			RemoveProp(hwnd, BBLEANSKIN_UNFOCUSTRANS);
		}

		if ( pInfo2 ) {
			//We're going to unroll any rolled up windows before we unload
			RECT rc;
			get_rect(hwnd, &rc);
			int height = rc.bottom - rc.top;
