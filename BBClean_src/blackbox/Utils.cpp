...
void log_printf(int flag, const char *fmt, ...)
{
    if ((Settings_LogFlag & flag) || 0 == (flag & 0x7FFF))
    {
        ...
        strcat(buffer, "\n");
		DWORD written;
		WriteFile(hlog_file, buffer, (DWORD)strlen(buffer), &written, NULL);
    }
}
...
// Out:
//===========================================================================
/* October 30, 2011 - Deprecated for nocd5's BBDrawText
...
int BBDrawText(HDC hDC, LPCTSTR lpString, int nCount, LPRECT lpRect, UINT uFormat, StyleItem* pSI){
    bool bShadow = (pSI->validated & VALID_SHADOWCOLOR) && (pSI->ShadowColor != ((COLORREF)-1));
    bool bOutline = (pSI->validated & VALID_OUTLINECOLOR) && (pSI->OutlineColor != ((COLORREF)-1));
	
	if (bShadow/* && !bOutline*/){ // draw shadow
        RECT rcShadow;
        ...
        //_SetRect(&rcShadow, lpRect->left + 1, lpRect->top + 1, lpRect->right + 1, lpRect->bottom + 1);
    ...
    if (bOutline){ // draw outline
        /*if (bShadow){ // draw shadow with outline
			//Draw thicker shadow to accent outline
			if (bShadow){ // draw shadow with outline
				RECT rcShadow;
				SetTextColor(hDC, pSI->ShadowColor);
			...
			/*	for (int i = 0; i < 3; i++){
		            for (int j = 0; j < 3; j++){
		                DrawText(hDC, lpString, nCount, &rcShadow, uFormat);
		                _OffsetRect(&rcShadow, 0, 1);
		            }
		            _OffsetRect(&rcShadow, 1, -3);
		        }
			}*/
			//Draw the outline
	        RECT rcOutline;
