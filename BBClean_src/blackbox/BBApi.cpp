...
bool SetTransparency(HWND hwnd, BYTE alpha)
{
	...
	if (wStyle2 & WS_EX_LAYERED) {
		...
		if ( stricmp(alphaMethod, "classic") == 0 ) {
			return 0 != pSetLayeredWindowAttributes(hwnd, 0, alpha, LWA_ALPHA);
		} else if ( stricmp(alphaMethod, "single") == 0 ) {
			//The alpha really does nothing here, it's either transparent, or it's not.
			return 0 != pSetLayeredWindowAttributes(hwnd, mStyle.MenuTransparentColor.Color, alpha, LWA_COLORKEY);
		} else if ( stricmp(alphaMethod, "mixture") == 0 ) {
			return 0 != pSetLayeredWindowAttributes(hwnd, mStyle.MenuTransparentColor.Color, alpha, LWA_COLORKEY|LWA_ALPHA);
		}
	}
