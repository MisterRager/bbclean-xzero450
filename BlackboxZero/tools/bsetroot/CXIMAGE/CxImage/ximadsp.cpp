...
bool CxImage::SplitCMYK(CxImage* c,CxImage* m,CxImage* y,CxImage* k)
{
	...
	for(long yy=0; yy<head.biHeight; yy++){
		for(long xx=0; xx<head.biWidth; xx++){
			...
			if (c) tmpc.SetPixelIndex(xx,yy,255-color.rgbRed);
			if (m) tmpm.SetPixelIndex(xx,yy,255-color.rgbGreen);
			if (y) tmpy.SetPixelIndex(xx,yy,255-color.rgbBlue);
			if (k) tmpk.SetPixelIndex(xx,yy,RGB2GRAY(color.rgbRed,color.rgbGreen,color.rgbBlue));
		}
	}
