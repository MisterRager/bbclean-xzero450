...
void CxImage::CreateFromHBITMAP(HBITMAP hbmp)
{
	...
	if (hbmp) { 
		...
		// create the image
        Create(bm.bmWidth, bm.bmHeight, bm.bmBitsPixel, CXIMAGE_FORMAT_BMP);
						...
						if (*pc!=*pct || !bTransparent){
							...
							if (a == 0) {			// Transparent, retain dest 
								...
								*pdst++= c.rgbRed; 
								...
								a1=(BYTE)~a;
								*pdst=(BYTE)((*pdst * a1 + a * c.rgbBlue)>>8), ++pdst;
								*pdst=(BYTE)((*pdst * a1 + a * c.rgbGreen)>>8), ++pdst;
								*pdst=(BYTE)((*pdst * a1 + a * c.rgbRed)>>8), ++pdst;
							} 
						} else {
							pdst+=3;
						}
