...
protected:
	...
	friend MenuItem* MakeMenuNOP(Menu *PluginMenu, LPCSTR Title, LPCSTR Icon);
	friend MenuItem* MakeMenuSEP(Menu *PluginMenu);
...
public:
	...
	virtual void Active(int bActive);
...
{
public:
	FolderItem(Menu* pSubMenu, const char* pszTitle, const char* pszIcon);
	void Paint(HDC hDC);
...
{
public:
	CommandItem(const char* pszCommand, const char* pszTitle, bool isChecked, const char* pszIcon);
	void Invoke(int button);
};
...
{
public:
	SpecialFolderItem(LPCSTR pszTitle, const char *path, struct pidl_node* pidl_list, const char  *optional_command, const char* pszIcon);
	~SpecialFolderItem();
	void ShowSubMenu(); 
	void Invoke(int button);
	friend class SpecialFolder;
...
{
public:
	ContextMenu (LPCSTR title, class ShellContext* w, HMENU hm, int m);
	~ContextMenu();
private:
