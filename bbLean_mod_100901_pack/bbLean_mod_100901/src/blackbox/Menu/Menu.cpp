...
static char g_szSearchString[32] = "";
//===========================================================================
// Menu constructor
...
bool Menu::Handle_Key(UINT msg, UINT wParam)
{
    ...
    bool bNormal = !Settings_menuKeyBindVI || m_bSearch;
    ...
    if (
        ...
        if (Settings_menuKeyBindVI){
            ...
            if (m_bSearch){
                if (wParam == VK_BACK){
                    g_szSearchString[strlen(g_szSearchString) - 1] = '\0';
                }
                else{
                    _strcat(g_szSearchString, chars);
                }
            }
            else if(wParam != 'n'){
                return false;
            ...
            if (char *p = strchr(g_szSearchString, '/')){
                strcpy(chars, p+1);
                char buf[32];
                _sprintf(buf, "%s|", g_szSearchString);
        ...
        // Shortcuts
        Item = !strcmp(g_szSearchString, "/") ? NULL : match_next_shortcut(chars);
        if (Item)
        {
            hilite(Item);
            if (!Settings_menuKeyBindVI){
    ...
    if (WM_KEYDOWN == msg)
    {
        ...
        shift = 0x8000 & GetKeyState(VK_SHIFT);
one_more:
        i = 1;
        switch(wParam)
        {
            ...
            case 'J': if (bNormal) break;
                goto case_vk_down;
            case 'K': if (bNormal) break;
                goto case_vk_up;
            case 'H': if (bNormal) break;
                goto case_vk_left;
            case 'L': if (bNormal) break;
                goto case_vk_right;
            case 'O': if (bNormal) break;
                goto case_vk_return;
            case 'G': if (bNormal) break;
                if (m_LastKey == 'G'){
                    if (!shift && !ctrl){
                        goto case_vk_home;
                    }
            ...
            case VK_RIGHT:
                if (m_pActiveItem){
                    if (MENUITEM_ID_VI & m_pActiveItem->m_ItemID){
                        m_pActiveItem->Key(msg, VK_RIGHT);
                        break;
                    }
                }
            ...
            case VK_SPACE:
                ...
                {
                    if (MENUITEM_ID_VI & m_pActiveItem->m_ItemID){
                        m_pActiveItem->Key(msg, VK_RETURN);
                        break;
                    }
...
//===========================================================================
// API: MakeMenuNOP with Icon
//===========================================================================

MenuItem* MakeMenuNOP(Menu *PluginMenu, LPCSTR Title, LPCSTR Icon)
{
    MenuItem *pItem = PluginMenu->AddMenuItem(new MenuItem(Title ? NLS1(Title) : ""));
    pItem->m_isNOP = MI_NOP_TEXT;
    pItem->m_pszIcon = new_str(Icon);
    pItem->GetIcon();
    return pItem;
}
