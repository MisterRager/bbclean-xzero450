...
{   &mStyle.Toolbar             , "toolbar",                (void*) &DefStyleA, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_MARGIN|HAS_FONT|VALID_JUSTIFY|DEFAULT_BORDER|DEFAULT_MARGIN|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
...
{   &mStyle.ToolbarLabel        , "toolbar.label",          (void*) &DefStyleB, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_MARGIN|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
{   &mStyle.ToolbarWindowLabel  , "toolbar.windowLabel",    (void*) &DefStyleA, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
{   &mStyle.ToolbarClock        , "toolbar.clock",          (void*) &DefStyleB, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },

{   &mStyle.MenuTitle           , "menu.title",             (void*) &DefStyleB, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_MARGIN|HAS_FONT|VALID_JUSTIFY|DEFAULT_MARGIN|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
{   &mStyle.MenuFrame           , "menu.frame",             (void*) &DefStyleA, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_PICCOLOR|VALID_MARGIN|HAS_FONT|VALID_JUSTIFY|DEFAULT_BORDER|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
{   &mStyle.MenuFrameExt        , "menu.frame.ext.*",       (void*) &mStyle.MenuFrame, C_STY,  VALID_TEXTCOLOR|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
{   &mStyle.MenuHilite          , "menu.hilite",            (void*) &DefStyleB, C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_PICCOLOR|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
...
// window.font:
{   &mStyle.windowLabelFocus    , "window",                 (void*) &mStyle.Toolbar,                C_STY,  HAS_FONT|VALID_JUSTIFY },

{   &mStyle.windowTitleFocus    , "window.title.focus",     (void*) &mStyle.Toolbar,                C_STY,  HAS_TEXTURE|DEFAULT_BORDER },
{   &mStyle.windowLabelFocus    , "window.label.focus",     (void*) &mStyle.ToolbarWindowLabel,     C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|HAS_FONT|VALID_JUSTIFY|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
...
{   &mStyle.windowLabelUnfocus  , "window.label.unfocus",   (void*) &mStyle.Toolbar,                C_STY,  HAS_TEXTURE|VALID_TEXTCOLOR|VALID_SHADOWCOLOR|VALID_OUTLINECOLOR },
...
{   &mStyle.MenuSeparator       , "menu.separator",         (void*) &DefStyleB,                     C_STY,  HAS_TEXTURE|DEFAULT_BORDER|VALID_SHADOWCOLOR },
...
{   &mStyle.windowButtonCloseFocus   , "window.button.close.focus",   (void*) &mStyle.windowButtonFocus,   C_STY, HAS_TEXTURE|VALID_PICCOLOR },
{   &mStyle.windowButtonClosePressed , "window.button.close.pressed", (void*) &mStyle.windowButtonPressed, C_STY, HAS_TEXTURE|VALID_PICCOLOR },
{   &mStyle.windowButtonCloseUnfocus , "window.button.close.unfocus", (void*) &mStyle.windowButtonUnfocus, C_STY, HAS_TEXTURE|VALID_PICCOLOR },
...
void * GetSettingPtr(int i)
{
    ...
    case SN_MENUVOLUMEHILITE        : return &mStyle.MenuVolumeHilite        ;
...
int Settings_ItemSize(int i)
{
    ...
    case SN_MENUVOLUMEHILITE        : return sizeof (StyleItem);
    ...
    do
    {
        if (cp->v & v)
        {
            ...
            case C_CO1:
                ...
                si->Color = CLR_INVALID != cr ? cr : def ->Color;
                break;

            case C_CO2:
                cr = ReadColorFromString(p);
                si->ColorTo = CLR_INVALID != cr ? cr : def ->ColorTo;
                break;

            case C_CO3:
                cr = ReadColorFromString(p);
                si->TextColor = CLR_INVALID != cr ? cr : def ->TextColor;
                break;

            case C_CO4:
                cr = ReadColorFromString(p);
                if (v & VALID_TEXTCOLOR)
                    si->foregroundColor = CLR_INVALID != cr ? cr : si->picColor;
                else
                    si->picColor = CLR_INVALID != cr ? cr : def->picColor;
                break;

            case C_CO5:
                cr = ReadColorFromString(p);
                si->ShadowColor = CLR_INVALID != cr ? cr : def ->ShadowColor;
                break;

            case C_CO6:
                cr = ReadColorFromString(p);
                si->OutlineColor = CLR_INVALID != cr ? cr : def ->OutlineColor;
                break;

            // --- Border & margin ---
            case C_BOCO:
                cr = ReadColorFromString(p);
                si->borderColor = CLR_INVALID != cr ? cr : mStyle.borderColor;
                break;
...
static void ReadStyle(const char *style)
{
    ...
    do
    {
        ...
        {
            case C_STY:
                if (0 == strcmp(p->rc_string, "menu.frame.ext.*")){
            ...
            case C_COL:
                ...
                *(COLORREF*)p->v = CLR_INVALID != cr ? cr : *(COLORREF*)def;
                break;
...
static struct rccfg ext_rccfg[] = {
    ...
    { "blackbox.menu.keyBindVI:",               C_BOL, (void*)false,    &Settings_menuKeyBindVI },
    { "blackbox.menu.iconSize:",                C_INT, (void*)16,       &Settings_menuIconSize },
    { "blackbox.menu.maxHeightRatio:",          C_INT, (void*)75,       &Settings_menuMaxHeightRatio },
    ...
    { "blackbox.separator.margin:",             C_INT, (void*)0,        &Settings_separatorMargin },
    { "blackbox.separator.style:",              C_STR, (void*)"",       &Settings_separatorStyle },
    // --------------------------------
