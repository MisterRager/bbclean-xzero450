...
void exec_button_action(WinInfo *WI, int n)
{
    ...
    switch(n)
    {
        ...
        case btn_Icon:
            POINT pt;
            pt.x = WI->S.rcClient.left;
            ClientToScreen(hwnd, &pt);

            RECT r;
            GetWindowRect(hwnd, &r);

            LPARAM lParam = MAKELPARAM(pt.x, r.top + mSkin.ncTop);
            DefWindowProc(hwnd, 0x0313, 0, lParam);
            break;
#endif
...
void DrawButton(WinInfo *WI, HDC hdc, RECT rc, int btn, bool state, bool active, bool pressed)
{
    ...
    if (btn != btn_Close){
        ...
        if (false == active) pSI = &mSkin.windowButtonCloseUnfocus;
        else if (pressed) pSI = &mSkin.windowButtonClosePressed;
        else pSI = &mSkin.windowButtonCloseFocus;
    }
...
void PaintAll(struct WinInfo* WI)
{
    ...
    HICON hIco = NULL;
    WI->has_icon = false;
    SendMessageTimeout(WI->hwnd, WM_GETICON, ICON_SMALL, 0, SMTO_ABORTIFHUNG|SMTO_NORMAL, 500, (DWORD_PTR*)&hIco);
    if (NULL==hIco) {
        hIco = (HICON)GetClassLong(WI->hwnd, GCLP_HICONSM);
        if (NULL==hIco) {
            SendMessageTimeout(WI->hwnd, WM_GETICON, ICON_BIG, 0, SMTO_ABORTIFHUNG|SMTO_NORMAL, 500, (DWORD_PTR*)&hIco);
            if (NULL==hIco) {
                hIco = (HICON)GetClassLong(WI->hwnd, GCLP_HICON);
            }
        }
    }
    if (hIco!=NULL) {
        int nIconSize = imin(16, mSkin.buttonSize);
        int nIconMargin = (title_height - nIconSize) / 2;

        DrawIconEx(hdc, left + nIconMargin, top + nIconMargin, hIco, nIconSize, nIconSize, 0, NULL, DI_NORMAL);

        rc.left += mSkin.buttonSize + mSkin.buttonMargin;
        label_left = rc.left;
        WI->has_icon = true;
    }
#endif
...
bool set_region(WinInfo *WI)
{
    ...
    if (WI->S.width != S.width)
    {
        ...
        int nPosWTF = offsetof(SkinStruct, windowTitleFocus)/sizeof(GradientItem);
        int nPosWFF = offsetof(SkinStruct, windowHandleFocus)/sizeof(GradientItem);
        int nPosWTU = offsetof(SkinStruct, windowTitleUnfocus)/sizeof(GradientItem);
        int nPosWFU = offsetof(SkinStruct, windowHandleUnfocus)/sizeof(GradientItem);
        DeleteBitmaps(WI, nPosWTF, nPosWFF - nPosWTF + 1);
        DeleteBitmaps(WI, nPosWTU, nPosWFU - nPosWTU + 1);
    }
...
int get_button (struct WinInfo *WI, int x, int y)
{
    ...
    else
    {
        n = btn_Caption;
#ifdef DRAW_ICON
        if (WI->has_icon) left += mSkin.buttonSize;
#endif
        int button_next = mSkin.buttonSize + mSkin.buttonSpace;
        for (int i = 0; i < 6; i++)
        {
#ifdef DRAW_ICON
            if (WI->has_icon && x < margin + mSkin.buttonSize){
                n = btn_Icon;
                break;
            }
#endif
...
{
    WinInfo *WI = get_WinInfo(hwnd); LRESULT result = 0; int n = 0;
    ...
    case WM_SETTEXT:
        ...
        {
            post_redraw(GetAncestor(hwnd, GA_ROOTOWNER));
            break;
        }
...
*/
    ...
    case WM_RBUTTONUP:
        if (btn_None == WI->capture_button)
            break;
        else if (WI->button_down == WI->capture_button){
            if (btn_Max == WI->capture_button)
                n = (wParam & MK_SHIFT) ? btn_VMax : btn_HMax;
            else if (btn_Min == WI->capture_button)
                n = btn_TMin;
        }
        else
            n = WI->button_down;
...
#ifdef DRAW_ICON
        if (n == btn_Icon) goto set_capture; // clicked in a icon
#endif
